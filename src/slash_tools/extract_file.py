...
def extract_file_command(
    ...
    return SlashToolResult(
        text=f"Extracted markdown saved:\n{output_path}",
...
def _extract_xlsx(source: Path, context: SlashToolContext) -> str:
    ...
    try:
        ...
        raise SlashToolError(f"Failed to open XLSX file {source.name}: {exc}") from exc
...
def _extract_pptx(source: Path, context: SlashToolContext) -> str:
    ...
    try:
        ...
        raise SlashToolError(f"Failed to open PPTX file {source.name}: {exc}") from exc
...
def _extract_pdf(source: Path, context: SlashToolContext) -> str:
    ...
    try:
        ...
        raise SlashToolError(f"Failed to open PDF file {source.name}: {exc}") from exc
...
def _extract_docx(source: Path, context: SlashToolContext) -> str:
    ...
    try:
        ...
        raise SlashToolError(f"Failed to open DOCX file {source.name}: {exc}") from exc
