...
from src.gemma_console_gui.attachment_handler import IMAGE_EXTENSIONS, validate_attachment_path
...
def build_image_analyze_content(path: str, instruction: str = "") -> list[dict[str, Any]]:
    try:
        image_path = validate_attachment_path(path)
    except Exception as exc:
        raise ImageAnalyzeError(str(exc)) from exc
    ...
    try:
        ...
        raise ImageAnalyzeError(f"Failed to read image file: {exc}") from exc
