...
Attach a workspace folder first, then run `/render_pptx` with no arguments. The attached folder is the working folder. Outputs are written to `llm_output` inside that folder, and intermediate artifacts are written to `llm_result`. The command supports `.pptx`, `.docx`, `.xlsx`, and `.pdf` files. Planner settings are loaded from `config.yaml`.
